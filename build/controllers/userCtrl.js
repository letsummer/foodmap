...
export const postJoin = async (req, res) => {
  ...
  const isExists = await Users.exists({
    userId
  });
...
export const postLogin = async (req, res) => {
  ...
  if (user && ok && userId === "admin") {
    ...
    .setHeader('Access-Control-Allow-Origin', `${process.env.CLIENT_URL}`).setHeader("Access-Control-Allow-Credentials", true).json({
      confirm_message: "관리자입니다.",
      session: req.session
    });
  }
  return res.status(200).json({
    confirm_message: "환영합니다."
  });
};
export const successLogin = (req, res) => {
  try {
    const data = req.session;
    console.log(`req.session 입니다.~: `, data);
    console.log(`###end###`);
